...
{
  // std::cout << "V: " << v << " W: " << w << " Multiway: " << multiway << " Bidomains: " << d.size() << std::endl;
  ...
  for (const Bidomain &old_bd : d)
  {
    ...
    // std::cout << "L: " << l << " R: " << r << " left_len: " << left_len << " right_len: " << right_len << " old left_len: " << old_bd.left_len << " old right_len: " << old_bd.right_len << " left_len_noedge: " << left_len_noedge << " right_len_noedge: " << right_len_noedge << " left_len && right_len: "<< (left_len && right_len) << std::endl;
           ...
           unsigned int matching_size_goal)
{
  ...
  if (current.size() > incumbent.size())
  {
    ...
    time_elapsed += (double)(lap.tv_nsec - start.tv_nsec) /
                    1000000000.0; // adding elapsed nanoseconds
    test_info.milestones.push_back({incumbent.size(), nodes, time_elapsed});
    if (!arguments.quiet && (nodes - incumbent.size() > 10 || incumbent.size() % 100 == 0))
    {
...
int main(int argc, char **argv)
{
  ...
  std::cout << "Induced subgraph done" << std::endl;
