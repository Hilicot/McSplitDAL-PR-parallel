...
struct Reward {
    ...
    gtype get_reward(int current_reward_policy, bool normalized) const;
...
struct DoubleQRewards : Rewards {
    ...
    mutex reward_mutex;
    int policy_switch_counter, current_reward_policy;

    DoubleQRewards(int n, int m) : Rewards(n, m), V(n), Q(n, vector<Reward>(m)), SingleQ(m),
                                   left_initial_sort_order(n, 0), right_initial_sort_order(n, 0),
                                   policy_switch_counter(arguments.reward_policy.policy_switch_counter),
                                   current_reward_policy(arguments.reward_policy.current_reward_policy) {};
